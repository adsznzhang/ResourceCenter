...
describe('Letterhead page', () => {
    ...
    it('has 4 letterheads', async () => {
        const page = visit('/letterhead');
        const letterheadCount = await page.evaluate(() => document.querySelectorAll('.letterhead-card').length).end()
    ...
    it('has 4 images from placeimg.com', async () => {
        ...
        const letterheadCount = await page.evaluate(sel => document.querySelectorAll(sel).length,selector).end();
    ...
    it('has at least one letterhead with a button', async () => {
        const page = visit('/letterhead');
        const buttonIsVisible = await pageXOffset.visible('.letterhead-card button').end();
        expect(buttonIsVisible).toEqual(true);
    });

    it('has some text before the letterheads', async () => {
        const page = visit('/letterhead');
        const textIsVisible = await page.visible('.container > .row:firs-child p').end();
